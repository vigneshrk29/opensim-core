...
int F_generic(const double** arg, double** res) {
    ...
    /// Body specifications
    pelvis = new OpenSim::Body("pelvis", 11.751210011095651,
            Vec3(-0.069729482228687481, 0, 0),
            Inertia(0.099778065737821386, 0.08453958682650041,
                    0.056197957258948036, 0, 0, 0));
    femur_r = new OpenSim::Body("femur_r", 9.2810312301269491,
            Vec3(0, -0.17281329846170712, 0),
            Inertia(0.13806543520006995, 0.036191910198076598,
                    0.14559252763442779, 0, 0, 0));
    tibia_r = new OpenSim::Body("tibia_r", 3.6993810916309018,
            Vec3(0, -0.20693800230674214, 0),
            Inertia(0.061783188539925718, 0.0062518702689210552,
                    0.062641288380758026, 0, 0, 0));
    /// Joints
    /// Ground-Pelvis transform
    SpatialTransform st_ground_pelvis;
    st_ground_pelvis[0].setCoordinateNames(
            OpenSim::Array<std::string>("pelvis_tilt", 1, 1));
    st_ground_pelvis[0].setFunction(new LinearFunction());
    st_ground_pelvis[0].setAxis(Vec3(0, 0, 1));
    st_ground_pelvis[1].setCoordinateNames(
            OpenSim::Array<std::string>("pelvis_list", 1, 1));
    st_ground_pelvis[1].setFunction(new LinearFunction());
    st_ground_pelvis[1].setAxis(Vec3(1, 0, 0));
    st_ground_pelvis[2].setCoordinateNames(
            OpenSim::Array<std::string>("pelvis_rotation", 1, 1));
    st_ground_pelvis[2].setFunction(new LinearFunction());
    st_ground_pelvis[2].setAxis(Vec3(0, 1, 0));
    st_ground_pelvis[3].setCoordinateNames(
            OpenSim::Array<std::string>("pelvis_tx", 1, 1));
    st_ground_pelvis[3].setFunction(new LinearFunction());
    st_ground_pelvis[3].setAxis(Vec3(1, 0, 0));
    st_ground_pelvis[4].setCoordinateNames(
            OpenSim::Array<std::string>("pelvis_ty", 1, 1));
    st_ground_pelvis[4].setFunction(new LinearFunction());
    st_ground_pelvis[4].setAxis(Vec3(0, 1, 0));
    st_ground_pelvis[5].setCoordinateNames(
            OpenSim::Array<std::string>("pelvis_tz", 1, 1));
    st_ground_pelvis[5].setFunction(new LinearFunction());
    st_ground_pelvis[5].setAxis(Vec3(0, 0, 1));
    /// Hip_r transform
    SpatialTransform st_hip_r;
    st_hip_r[0].setCoordinateNames(
            OpenSim::Array<std::string>("hip_flexion_r", 1, 1));
    st_hip_r[0].setFunction(new LinearFunction());
    st_hip_r[0].setAxis(Vec3(0, 0, 1));
    st_hip_r[1].setCoordinateNames(
            OpenSim::Array<std::string>("hip_adduction_r", 1, 1));
    st_hip_r[1].setFunction(new LinearFunction());
    st_hip_r[1].setAxis(Vec3(1, 0, 0));
    st_hip_r[2].setCoordinateNames(
            OpenSim::Array<std::string>("hip_rotation_r", 1, 1));
    st_hip_r[2].setFunction(new LinearFunction());
    st_hip_r[2].setAxis(Vec3(0, 1, 0));

    /// Knee_r transform
    SpatialTransform st_knee_r;
    st_knee_r[0].setCoordinateNames(
	...
	st_knee_r[3].setCoordinateNames(
            OpenSim::Array<std::string>("knee_angle_r", 1, 1));
    double coefficientsTr1[5] = { -0.0007, 0.0013, 0.0028, -0.0063, -0.0036};
    auto* tr1 = new PolynomialFunction();
    tr1->setCoefficients(SimTK::Vector(5, coefficientsTr1));
    st_knee_r[3].setFunction(tr1);
    st_knee_r[3].setAxis(Vec3(1, 0, 0));

    st_knee_r[4].setCoordinateNames(
            OpenSim::Array<std::string>("knee_angle_r", 1, 1));
    double coefficientsTr2[4] = {0.0001, -0.0031, 0.0058, -0.3956};
    auto* tr2 = new PolynomialFunction();
    tr2->setCoefficients(SimTK::Vector(4, coefficientsTr2));
    st_knee_r[4].setFunction(tr2);
    st_knee_r[4].setAxis(Vec3(0, 1, 0));

 //   st_knee_r[3].setCoordinateNames(
 //           OpenSim::Array<std::string>("knee_angle_r", 1, 1));
	//double coefficientsTr1[5] = {-0.0036, -0.0063, 0.0028, 0.0013, -0.0007};
 ...
 //   st_knee_r[3].setAxis(Vec3(1, 0, 0));
	...
	//st_knee_r[4].setCoordinateNames(
 //           OpenSim::Array<std::string>("knee_angle_r", 1, 1));
	//double coefficientsTr2[4] = {-0.3956, 0.0058, -0.0031, 0.0001};
 //   auto* tr2 = new MultivariatePolynomialFunction();
 //   tr2->setDimension(1);
 //   tr2->setOrder(3);
 //   tr2->setCoefficients(SimTK::Vector(4, coefficientsTr2));
 //   st_knee_r[4].setFunction(tr2);
 ...
 //   st_knee_r[4].setAxis(Vec3(0, 1, 0));
    ...
    ground_pelvis = new CustomJoint("ground_pelvis", model->getGround(),
            Vec3(0), Vec3(0), *pelvis, Vec3(0), Vec3(0), st_ground_pelvis);
    hip_r = new CustomJoint("hip_r", *pelvis,
            Vec3(-0.069729482228687481, -0.065192627656523949,
                    0.08235377321209908),
            Vec3(0), *femur_r, Vec3(0), Vec3(0), st_hip_r);
    ...
    model->print("testPolynomial.xml");
    ...
    for (int i = 0; i < model->getBodySet().getSize(); ++i) {
        model->getMatterSubsystem().addInStationForce(*state,
                ...
                model->getBodySet().get(i).getMass() * gravity,
                appliedBodyForces);
    }
    /// knownUdot
