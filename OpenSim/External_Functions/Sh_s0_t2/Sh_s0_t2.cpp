...
constexpr int NR = ndof + 1 * NLambda + 6; // # outputs
...
int F_generic(const T** arg, T** res) {
    ...
    res[0][0 + NU + 1 * NLambda] = value<T>(qdotCorr[3]);
    res[0][1 + NU + 1 * NLambda] = value<T>(qdotCorr[4]);
    res[0][2 + NU + 1 * NLambda] = value<T>(qdotCorr[5]);
    res[0][3 + NU + 1 * NLambda] = value<T>(qdotCorr[6]);
    res[0][4 + NU + 1 * NLambda] = value<T>(qdotCorr[7]);
    res[0][5 + NU + 1 * NLambda] = value<T>(qdotCorr[8]);
...
int main() {
    ...
    for (int i = 0; i < NLambda; ++i) gamma[i] <<= -1;
