...
constexpr int NR = ndof+6+6+6;   // # residual torques + # GRFs + # joint origins
...
int F_generic(const T** arg, T** res) {
	...
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc + nc + nc + nc] = value<T>(GRM_r[1][i]);       /// GRM_r
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc + nc + nc + nc + nc] = value<T>(GRM_l[1][i]);  /// GRM_l
	}
