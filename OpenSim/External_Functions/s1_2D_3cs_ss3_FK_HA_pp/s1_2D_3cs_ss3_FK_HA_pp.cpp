...
constexpr int NR = ndof+6+2;  // # residual torques + GRFs + GRMs
...
int F_generic(const T** arg, T** res) {
    ...
    knee_l = new CustomJoint("knee_l", *femur_l, Vec3(-0.00442945, -0.38962777, 0), Vec3(0), *tibia_l, Vec3(0), Vec3(0), st_knee_l);
    knee_r = new CustomJoint("knee_r", *femur_r, Vec3(-0.00442945, -0.38962777, 0), Vec3(0), *tibia_r, Vec3(0), Vec3(0), st_knee_r);
    ...
    const SimTK::Vec3 translation_head(0, 0.55, 0);
    SimTK::Vec3 linAcc_head = getLinearAccelerationIMUInG(*model, *state, A_GB, "torso", gravity, translation_head);
    ...
    for (int i = 0; i < 2; ++i) {
            res[0][i + NU]     = value<T>(GRF_r[i]);  // GRF_r (x and y)
            res[0][i + NU + 2] = value<T>(GRF_l[i]);  // GRF_l (x and y)
    }
    res[0][NU + 4] = value<T>(MOM_r[2]);  // GRM_r (z only)
    res[0][NU + 5] = value<T>(MOM_l[2]);  // GRM_l (z only)
    int nc = 2;
    for (int i = 0; i < nc; ++i) res[0][i + NU + 6] = value<T>(linAcc_head[i]); // x and y components (first 2)
