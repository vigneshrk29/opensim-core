...
constexpr int NR = ndof + 2 * NLambda + 6 + 2*3; // # outputs
...
int F_generic(const T** arg, T** res) {
    ...
    /// 4. IMU data
    int nc = 3;
    for (int i = 0; i < nc; ++i) res[0][i + NU + 2 * NLambda + 6 + 0 * nc] = value<T>(angVel_radius_imu_inB[i]);
    for (int i = 0; i < nc; ++i) res[0][i + NU + 2 * NLambda + 6 + 1 * nc] = value<T>(linAcc_radius_imu_inB[i]);
