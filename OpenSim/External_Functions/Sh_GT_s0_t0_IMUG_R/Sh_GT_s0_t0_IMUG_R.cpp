...
constexpr int NR = ndof + 3 * NLambda + 6 + 9; // # outputs
...
int F_generic(const T** arg, T** res) {
    ...
    /// 4. IMU data
    int nc = 3;
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 0 * nc] = value<T>(angVel_radius_imu_inG[i]);
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 1 * nc] = value<T>(linAcc_radius_imu_inG[i]);
    /// 5. Orientation data (in ground frame) expressed as body fixed XYZ Euler angles
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 2 * nc] = value<T>(EA_radius_imu_inG[i]);
