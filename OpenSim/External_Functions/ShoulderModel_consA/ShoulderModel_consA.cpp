...
constexpr int NP = 3;			// # multipliers
...
int F_generic(const T** arg, T** res) {
	...
	/// Multipliers
	for (int i = 0; i < NP; ++i) multipliers[i] = p[i];
	...
	model->getMatterSubsystem().calcConstraintForcesFromMultipliers(*state, -multipliers, constraintBodyForces, constraintMobilityForces);

	// Makes sense based on calcResidualForce
	appliedBodyForces += constraintBodyForces;
	appliedMobilityForces += constraintMobilityForces;

	//std::cout << multipliers << std::endl;
	//std::cout << constraintBodyForces << std::endl;
	//std::cout << constraintMobilityForces << std::endl;

	//std::cout << appliedBodyForces + constraintBodyForces << std::endl;
	//std::cout << appliedBodyForces + constraintBodyForces << std::endl;
