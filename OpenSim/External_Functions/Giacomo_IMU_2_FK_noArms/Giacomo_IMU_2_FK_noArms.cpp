...
constexpr int NR = ndof + 16*3 + 8*4;    // # residual torques + # joint origins
...
int F_generic(const T** arg, T** res) {
    ...
    const SimTK::Vec3 orientation_XYZ_pelvis_imu(-1.334791708492248, -1.1869361939759826, 0.23826934527288943);
    const SimTK::Vec3 orientation_XYZ_torso_imu(-2.5668638689757075, 1.0767487637057662, -2.1139193273455099);
    const SimTK::Vec3 orientation_XYZ_femur_l_imu(2.9587270691211813, 0.030194353344661289, -1.6129207416848623);
    const SimTK::Vec3 orientation_XYZ_femur_r_imu(0.32610717805514144, 0.68395575043853107, 1.2406420823103426);
    const SimTK::Vec3 orientation_XYZ_tibia_l_imu(1.350932249146384, 1.532278687276029, 0.19705240510606822);
    const SimTK::Vec3 orientation_XYZ_tibia_r_imu(-3.0905037248384777, -0.34678142507539517, -1.5866908668144737);
    const SimTK::Vec3 orientation_XYZ_calcn_l_imu(-2.4262640628918826, 0.20795746231198739, -2.5304207818461504);
    const SimTK::Vec3 orientation_XYZ_calcn_r_imu(-1.159590406192907, 0.7253463692310923, -2.8557109596577566);
    ...
    SimTK::Quaternion orientation_quat_pelvis_imu_G(orientation_rot_pelvis_imu_G);
    SimTK::Quaternion orientation_quat_torso_imu_G(orientation_rot_torso_imu_G);
    SimTK::Quaternion orientation_quat_femur_l_imu_G(orientation_rot_femur_l_imu_G);
    SimTK::Quaternion orientation_quat_femur_r_imu_G(orientation_rot_femur_r_imu_G);
    SimTK::Quaternion orientation_quat_tibia_l_imu_G(orientation_rot_tibia_l_imu_G);
    SimTK::Quaternion orientation_quat_tibia_r_imu_G(orientation_rot_tibia_r_imu_G);
    SimTK::Quaternion orientation_quat_calcn_l_imu_G(orientation_rot_calcn_l_imu_G);
    SimTK::Quaternion orientation_quat_calcn_r_imu_G(orientation_rot_calcn_r_imu_G);
    ...
    for (int i = 0; i < nVec4; ++i) res[0][i + NU + 16*nc +0*nVec4] = value<T>(orientation_quat_pelvis_imu_G[i]);
    for (int i = 0; i < nVec4; ++i) res[0][i + NU + 16*nc +1*nVec4] = value<T>(orientation_quat_torso_imu_G[i]);
    for (int i = 0; i < nVec4; ++i) res[0][i + NU + 16*nc +2*nVec4] = value<T>(orientation_quat_femur_l_imu_G[i]);
    for (int i = 0; i < nVec4; ++i) res[0][i + NU + 16*nc +3*nVec4] = value<T>(orientation_quat_femur_r_imu_G[i]);
    for (int i = 0; i < nVec4; ++i) res[0][i + NU + 16*nc +4*nVec4] = value<T>(orientation_quat_tibia_l_imu_G[i]);
    for (int i = 0; i < nVec4; ++i) res[0][i + NU + 16*nc +5*nVec4] = value<T>(orientation_quat_tibia_r_imu_G[i]);
    for (int i = 0; i < nVec4; ++i) res[0][i + NU + 16*nc +6*nVec4] = value<T>(orientation_quat_calcn_l_imu_G[i]);
    for (int i = 0; i < nVec4; ++i) res[0][i + NU + 16*nc +7*nVec4] = value<T>(orientation_quat_calcn_r_imu_G[i]);

    //for (int i = 0; i < nVec4; ++i) res[0][i +0*nVec4] = value<T>(orientation_quat_torso_imu_G[i]);
