...
constexpr int NR = ndof + 3 * NLambda + 6 + 6; // # outputs
...
int F_generic(const T** arg, T** res) {
    ...
    const SimTK::Rotation R_GB = model->getBodySet().get("radius").getMobilizedBody().getBodyTransform(*state).R();
    // Angular velocity measured with respect to the ground frame and expressed in the BODY frame.
    SimTK::Vec3 angVel_radius_imu_inB = ~R_GB * angVel_radius_imu_inG;
    ...
    /// 4. IMU data
    int nc = 3;
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 0 * nc] = value<T>(angVel_radius_imu_inB[i]);
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 1 * nc] = value<T>(linAcc_radius_imu_inB[i]);
