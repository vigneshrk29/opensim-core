...
int F_generic(const T** arg, T** res) {
    ...
    //std::cout << model->getStateVariableValues(*state) << std::endl;
    ...
    res[0][0 + NU + 3 * NLambda] = value<T>(qdotCorr[3]);
    res[0][1 + NU + 3 * NLambda] = value<T>(qdotCorr[4]);
    res[0][2 + NU + 3 * NLambda] = value<T>(qdotCorr[5]);
    ...
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 6 * nc + nc * nc] = value<T>(EA_radius_imu_inG[i]);
...
int main() {
    ...
    x[6] <<= (10 * SimTK::Pi / 180).value();
    x[7] <<= 0;
    x[8] <<= (20 * SimTK::Pi / 180).value();
    x[9] <<= 0;
