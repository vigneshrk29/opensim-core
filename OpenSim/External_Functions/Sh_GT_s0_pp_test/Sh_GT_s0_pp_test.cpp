...
constexpr int NR = ndof + 3 * NLambda + 6 + 2 * 3 + 4 * 3 + 9 + 3; // # outputs
...
std::unordered_map<std::string, int> createSystemYIndexMap(
    const Model& model) {
...
int F_generic(const double** arg, double** res) {
    ...
    std::vector<double> lambda(arg[2], arg[2] + NLambda);
    std::vector<double> gamma(arg[3], arg[3] + NLambda);
    ...
    Vector in_u(model->getMatterSubsystem().getNU(*state));
    for (int i = 0; i < in_u.size(); ++i) in_u[i] = 1;
    Vector out_q(model->getMatterSubsystem().getNU(*state));
    model->getMatterSubsystem().multiplyByN(*state, false, in_u, out_q);
    std::cout << out_q << std::endl;

    auto svValues = model->getStateVariableValues(*state);
    auto svNames = model->getStateVariableNames();

    for (int i = 0; i < ndof*2; i += 2) {
        std::cout << svNames.get(i) << ": " << svValues.get(i) << std::endl;
    }
    ...
    /// 2. Contraint errors.
    for (int i = 0; i < NLambda; ++i) res[0][i + NU + 0 * NLambda] = (qerr[i]);
    for (int i = 0; i < NLambda; ++i) res[0][i + NU + 1 * NLambda] = (uerr[i]);
    for (int i = 0; i < NLambda; ++i) res[0][i + NU + 2 * NLambda] = (aerr[i]);
    /// 3. Velocity correctors.
    /// TODO: should sort of automate this.
    res[0][0 + NU + 3 * NLambda] = (qdotCorr[3]);
    res[0][1 + NU + 3 * NLambda] = (qdotCorr[4]);
    res[0][2 + NU + 3 * NLambda] = (qdotCorr[5]);
    res[0][3 + NU + 3 * NLambda] = (qdotCorr[6]);
    res[0][4 + NU + 3 * NLambda] = (qdotCorr[7]);
    res[0][5 + NU + 3 * NLambda] = (qdotCorr[8]);
    /// 4. Station locations for verification.
    int nc = 3;
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 0 * nc] = (AC_clavicle_location[i]);
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 1 * nc] = (AC_scapula_location[i]);
    /// 5. IMU data (in body frame)
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 2 * nc] = (angVel_radius_imu_inB[i]);
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 3 * nc] = (linAcc_radius_imu_inB[i]);
    /// 6. IMU data (in ground frame)
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 4 * nc] = (angVel_radius_imu_inG[i]);
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 5 * nc] = (linAcc_radius_imu_inG[i]);
    /// 7. Orientation data (in ground frame): Rotation matrices and body fixed XYZ Euler angles
    for (int i = 0; i < nc; ++i) for (int j = 0; j < nc; ++j) res[0][i*nc + j + NU + 3 * NLambda + 6 + 6 * nc] = (R_radius_imu_inG[i][j]);
    for (int i = 0; i < nc; ++i) res[0][i + NU + 3 * NLambda + 6 + 6 * nc + nc * nc] = (EA_radius_imu_inG[i]);
...
int main() {
    ...
    const double* Recorder_arg[n_in] = { x,u,lambda,gamma };
