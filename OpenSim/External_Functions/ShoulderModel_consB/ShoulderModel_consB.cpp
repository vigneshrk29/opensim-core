...
constexpr int NR = ndof + 6;        // # residual torques
...
int F_generic(const T** arg, T** res) {
	...
	/// Stations
	AC_clavicle = new Station(*clavicle, Vec3(-0.029239999999999999, 0.020240000000000001, 0.12005));
	AC_scapula = new Station(*scapula, Vec3(-0.01357, 0.00011, -0.01523));
	...
	int nc = 3;
	for (int i = 0; i < nc; ++i) res[0][i + NU + 0 * nc] = value<T>(AC_clavicle_location[i]);
	for (int i = 0; i < nc; ++i) res[0][i + NU + 1 * nc] = value<T>(AC_scapula_location[i]);
