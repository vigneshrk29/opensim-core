...
constexpr int nCoordinates_wLocked = 33;
constexpr int NX = nCoordinates * 2;
constexpr int NU = nCoordinates;
constexpr int NR = nCoordinates + 3 * 6;;
...
int F_generic(const T** arg, T** res) {
	...
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc + nc + nc + nc] = value<T>(GRM_r[1][i]);       /// GRF_r
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc + nc + nc + nc + nc] = value<T>(GRM_l[1][i]);  /// GRF_l
	}
