...
int main()
{
    ...
    SimTK::Vec4 test_quat_val(0.7417869684642331,-0.2817079813335129,-0.4720628893932242,-0.3841215108886257);
    SimTK::Quaternion test_quat(test_quat_val);
    SimTK::Rotation test_rot(test_quat);
    std::cout << test_rot << std::endl;

    SimTK::Vec3 p(0);
    SimTK::Transform a(test_rot, p);

    SimTK::Rotation r = a.R();
    SimTK::Vec3 orientation = r.convertRotationToBodyFixedXYZ();

    SimTK::Rotation R_pelvis_imu(
            SimTK::BodyOrSpaceType::BodyRotationSequence,
            orientation[0], SimTK::XAxis,
            orientation[1], SimTK::YAxis,
            orientation[2], SimTK::ZAxis);
    std::cout << R_pelvis_imu << std::endl;

    //// Calibrate model and compare result to standard
    //IMUPlacer imuPlacer("imuPlacer_test.xml");
    //imuPlacer.run();
    return 0;
}
